...
for (sam_method in c("all_genes", "per_gene")) {
  ...
  for (gene_method in c("high", "medium", "low")) {
    ...
    simdata <- simulationWrapper.sampleGenes(data, Nsim = Nsim, Ngenes = 1000,
                                Nsamples = Nsam,
                                sample_method = sam_method,
                                sampleGenes_method = gene_method,
                                pi0 = pi0,
    ...
    saveRDS(simdata,
      file = paste0(args[2], data_name, "/", data_name,".", sam_label, ".", gene_method, ".",
